...
try {
	...
	switch ($type) {
		case "save_data";
			if (!isset($_GET['formID'])) exit("parameter formID missing");
			$form_id = $_GET['formID'];
		...
		case "load_UI":
			...
			foreach($result as $entry) {
				echo '<tr><td style="width: 90%"><a href="#" onclick="galaxytool.FormSaveHelper.loadFormData(\''.$form_id.'\',\''.$entry['save_id'].'\')">'.$entry['save_id'].'</a></td>'.
				     '<td><img src="../images/trash.png" onclick="galaxytool.FormSaveHelper.deleteFormData(\''.$form_id.'\',\''.$entry['save_id'].'\')" style="cursor: pointer;"></td></tr>';
			}
			echo "</table>";
		...
		case "save_UI":
			...
			<button dojoType="dijit.form.Button" type="button" id="formsave_button" onClick="galaxytool.FormSaveHelper.validateSaveScreen('<?php echo $form_id; ?>')"><?php echo GENERAL_SAVE; ?></button>
